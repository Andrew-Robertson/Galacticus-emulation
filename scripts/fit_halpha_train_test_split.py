...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fit a Gaussian-process emulator on a subset of Galacticus evaluations and test it "
            "on the complementary set, for dust-attenuated Halpha LF outputs."
        )
    )
    ...
    parser.add_argument("--bin-indices", type=int, nargs="+", required=True)
    ...
    parser.add_argument(
        ...
        default="white",
        choices=["white"],
        help="Use an optimized WhiteKernel for model-side training noise.",
    )
...
def _kernel_known_noise(n_dim: int):
    return ConstantKernel(4.0, (1.0e-3, 1.0e3)) * Matern(
        length_scale=np.asarray([1.0, 0.5, 1.5, 1.0, 3.0, 10.0], dtype=float)[:n_dim],
        length_scale_bounds=(1.0e-2, 1.0e2),
        nu=2.5,
...
def main() -> None:
    ...
    table, all_output_columns = _load_table(campaign_root, args.table_filename, args.sobral_label)
    table = _apply_filters(table, args.max_abs_delta, args.max_attenuation_scatter)
    output_columns = [all_output_columns[index] for index in args.bin_indices]
    ...
    x_train = transform_to_prior_quantiles(INPUT_PARAMETER_SPECS, train[INPUT_COLUMNS].to_numpy(dtype=float))
    x_test = transform_to_prior_quantiles(INPUT_PARAMETER_SPECS, test[INPUT_COLUMNS].to_numpy(dtype=float))
    ...
    y_test, _, n_floored_test, n_clipped_test = _log10_with_floor(
        test[output_columns].to_numpy(dtype=float),
        args.min_log10_lf,
    )
    ...
    suffix = (
        f"_split_{args.sobral_label.lower()}_trainevals{args.n_train_evals}_traindraws{args.train_draws_per_eval}"
    )
    if args.output_tag:
        suffix += f"_{args.output_tag}"

    metrics = _metrics(y_test, y_pred, y_pred_std, output_columns)
    metrics_path = emulator_root / f"gp_split_metrics{suffix}.csv"
    metrics.to_csv(metrics_path, index=False)

    predictions = test[["evaluation_id", "dust_draw_index", *INPUT_COLUMNS]].copy()
    for bin_index, output_column in enumerate(output_columns):
    ...
    predictions_path = emulator_root / f"gp_split_predictions{suffix}.csv"
    ...
    summary = {
        ...
        "noise_floor_dex": args.noise_floor_dex,
    ...
    summary_path = emulator_root / f"gp_split_summary{suffix}.json"
    summary_path.write_text(json.dumps(summary, indent=2) + "\n")

    parity_path = figures_root / f"gp_split_parity{suffix}.png"
